;;; embark-by-type.el --- Type-aware embark navigation + picker UX -*- lexical-binding: t; -*-

;; Copyright (C) 2026 Charlie Holland

;; Author: Charlie Holland <charliebkr707@gmail.com>
;; Maintainer: Charlie Holland <charliebkr707@gmail.com>
;; URL: https://github.com/<TBD>/embark-by-type
;; Version: 0.1.0
...
;;
;;   `embark-by-type-nav-*'   move by current target's type (next /
;;                            prev / beg / end)
;;   `embark-by-type-pick-*'  pick a type (consult + preview, or
;;                            single-key transient) or a visible
;;                            instance of one (consult or avy)
;;   `embark-by-type-act-*'   act on the captured target -- focus,
;;                            highlight other instances, select as
;;                            region, narrow
;;
;; Plus:
;;   - `embark-by-type-sort-by-bounds-mode' -- innermost-first cycle
;;     order on `embark--targets'.
;;   - `embark-by-type-back-cycle-mode' -- C-, in the embark menu
;;     prompter cycles backward.
;;   - `embark-by-type-deftap-finder' macro -- wrap any
...
;; One-call setup:
;;
;;   (embark-by-type-setup)            ; capture + sort + bindings
;;
...
(defgroup embark-by-type nil
  "Type-aware embark navigation + picker UX."
  :group 'convenience
  :prefix "embark-by-type-")
...
(defvar-keymap embark-by-type-defun-map
...
(defmacro embark-by-type-deftap-finder (thing &optional type)
  "Define an embark target finder for `thing-at-point' THING.

Generates a finder named `embark-by-type-target-<thing>-at-point' that
...
Adds the finder to `embark-target-finders'."
  (let* ((thing-sym (if (consp thing) (eval thing) thing))
         ...
         (name (intern (format "embark-by-type-target-%s-at-point" thing-sym))))
    `(progn
       (defun ,name ()
...
(defcustom embark-by-type-word-finder-modes
  '(text-mode org-mode emacs-lisp-mode)
  "Major modes (and derivatives) where `embark-by-type-target-word-at-point' fires.
...
unconditionally included regardless of this list."
  :type '(repeat symbol)
  :group 'embark-by-type)

(defcustom embark-by-type-word-finder-extra-modes
  '(eww-mode help-mode Info-mode Man-mode)
  "Extra modes (exact match, not derived) where the word finder fires."
  :type '(repeat symbol)
  :group 'embark-by-type)

(defun embark-by-type-target-word-at-point ()
  "Embark target finder for `word'.

Active in prose buffers (text/org/eww/help/Info/Man) AND elisp,
per `embark-by-type-word-finder-modes' +
`embark-by-type-word-finder-extra-modes'."
  (when (and (not (or (minibufferp)
                      (derived-mode-p 'completion-list-mode
                                      'embark-collect-mode)))
             (or (apply #'derived-mode-p embark-by-type-word-finder-modes)
                 (memq major-mode embark-by-type-word-finder-extra-modes)))
...
(defcustom embark-by-type-sort-by-bounds-p t
  "When non-nil, `embark-by-type-sort-by-bounds-mode' sorts embark's
target cycle by bounds size (ascending)."
  :type 'boolean
  :group 'embark-by-type)

(defvar embark-by-type--sort-reverse nil
  "Dynamic flip controlling sort direction.  Bound by
`embark-by-type-act-contract'.")

(defun embark-by-type--sort-targets-by-bounds (targets)
  "Sort TARGETS by bounds size ascending (innermost first).
Targets without bounds keep their relative order at the end of the
list.  When `embark-by-type--sort-reverse' is non-nil, the cycle
order flips (outermost first)."
  (if embark-by-type-sort-by-bounds-p
             ...
             (sorted (sort with-bounds
                           (lambda (a b)
                             (let* ((a-bounds (plist-get a :bounds))
                               ...
                               (if embark-by-type--sort-reverse
                                   (> a-size b-size)
                                 (< a-size b-size)))))))
...
;;;###autoload
(define-minor-mode embark-by-type-sort-by-bounds-mode
  "Sort embark's target cycle by bounds size (innermost first).
Installs a `:filter-return' advice on `embark--targets' that
post-sorts the target list per `embark-by-type-sort-by-bounds-p'."
  :global t
  :group 'embark-by-type
  (if embark-by-type-sort-by-bounds-mode
      (advice-add 'embark--targets :filter-return
                  #'embark-by-type--sort-targets-by-bounds)
    (advice-remove 'embark--targets
                   #'embark-by-type--sort-targets-by-bounds)))

;;;###autoload
(defun embark-by-type-act-contract ()
...
noise (e.g. acting on the whole defun rather than the call inside)."
  (interactive)
  (let ((embark-by-type--sort-reverse t))
...
(defun embark-by-type-back-cycle ()
...
embark prompter UI."
  (interactive)
  (user-error
   "`embark-by-type-back-cycle' is meant for the embark prompter only"))

(defun embark-by-type--keymap-prompter-back-cycle (orig keymap update)
  "Translate `embark-by-type-back-cycle' returns into backward
`embark-cycle' by forcing `prefix-arg' to -1."
  (let ((cmd (funcall orig keymap update)))
    (if (eq cmd 'embark-by-type-back-cycle)
        (progn (setq prefix-arg -1) 'embark-cycle)
      cmd)))

(defun embark-by-type--reset-prefix-arg-after-rotate (&rest _)
  "Reset `prefix-arg' after rotation -- one-shot prefix semantics
inside the embark-act cycle loop."
  (setq prefix-arg nil))

;;;###autoload
(define-minor-mode embark-by-type-back-cycle-mode
...
after each rotation so the direction is per-press, not sticky."
  :global t
  :group 'embark-by-type
  (if embark-by-type-back-cycle-mode
      (progn
        (define-key embark-general-map (kbd "C-,")
                    #'embark-by-type-back-cycle)
        (advice-add 'embark-keymap-prompter :around
                    #'embark-by-type--keymap-prompter-back-cycle)
        (advice-add 'embark--rotate :after
                    #'embark-by-type--reset-prefix-arg-after-rotate))
...
(defcustom embark-by-type-nav-type-map
  '((general . word)
    (identifier . symbol)
    (expression . sexp)
    (defun . defun)
...
(defvar embark-by-type-last-target-type nil
  "Type of the most recent embark target.
Maintained by `embark-by-type-capture-mode'.  Read by `nav-*' / `act-*'
commands.  The `last-' prefix signals: \"set by hook, not by user\".")

(defvar embark-by-type-last-target-bounds nil
  "Bounds of the most recent embark target, or nil if unbounded.
Maintained by `embark-by-type-capture-mode'.")

(defun embark-by-type--capture-target (&rest plist)
  "Pre-action hook: capture target type and bounds for nav/act commands."
  (setq embark-by-type-last-target-type (plist-get plist :type)
        embark-by-type-last-target-bounds (plist-get plist :bounds)))

(defun embark-by-type--capture-on-rotate (rotated-targets)
...
unchanged."
  (when rotated-targets
    (let ((target (car rotated-targets)))
      (setq embark-by-type-last-target-type (plist-get target :type)
            embark-by-type-last-target-bounds (plist-get target :bounds))))
  rotated-targets)

;;;###autoload
(define-minor-mode embark-by-type-capture-mode
...
during cycling.  Required by every `embark-by-type-nav-*' and
`embark-by-type-act-*' command -- those guard with `user-error'."
  :global t
  :group 'embark-by-type
  (if embark-by-type-capture-mode
      (progn
        ;; (1) Pre-action hook -- idempotent install.
        (let ((hooks (alist-get :always embark-pre-action-hooks)))
          (unless (memq #'embark-by-type--capture-target hooks)
            (setf (alist-get :always embark-pre-action-hooks)
                  (cons #'embark-by-type--capture-target hooks))))
        ;; (2) Rotate advice.
        (advice-add 'embark--rotate :filter-return
                    #'embark-by-type--capture-on-rotate))
    ;; Clean toggle off -- both writers removed.
    (setf (alist-get :always embark-pre-action-hooks)
          (delq #'embark-by-type--capture-target
                (alist-get :always embark-pre-action-hooks)))
    (advice-remove 'embark--rotate
                   #'embark-by-type--capture-on-rotate)))

(defun embark-by-type--require-capture-mode ()
  "Signal a `user-error' if `embark-by-type-capture-mode' is off."
  (unless embark-by-type-capture-mode
    (user-error
     "Enable `embark-by-type-capture-mode' first (the nav/act family needs it)")))


;;;; Nav commands
;; ----------------------------------------------------------------

(defun embark-by-type--nav (n)
  "Move N instances forward (negative = back) of current target's type."
  (let* ((type embark-by-type-last-target-type)
         (bounds embark-by-type-last-target-bounds)
         (thing (alist-get type embark-by-type-nav-type-map type)))
    (cond
     ((null type)
...
;;;###autoload
(defun embark-by-type-nav-next ()
  "Move to next instance of current embark target's type."
  (interactive)
  (embark-by-type--require-capture-mode)
  (embark-by-type--nav 1))

;;;###autoload
(defun embark-by-type-nav-prev ()
...
;;;###autoload
(defun embark-by-type-nav-beg ()
  "Move point to the start of the current embark target's bounds."
  (interactive)
  (embark-by-type--require-capture-mode)
  (if embark-by-type-last-target-bounds
      (goto-char (car embark-by-type-last-target-bounds))
    (message "No embark target bounds captured")))

;;;###autoload
(defun embark-by-type-nav-end ()
...
boundary."
  ...
  (embark-by-type--require-capture-mode)
  (if embark-by-type-last-target-bounds
      (let ((beg (car embark-by-type-last-target-bounds))
            (end (cdr embark-by-type-last-target-bounds)))
...
(defun embark-by-type--expression-bounds ()
...
;;;###autoload
(defun embark-by-type-act-focus ()
  ...
  (cond
   ...
   ((null embark-by-type-last-target-type)
    (message "No embark target captured yet"))
   (t
    (let* ((type embark-by-type-last-target-type)
           (thing (alist-get type embark-by-type-nav-type-map type)))
...
;;;###autoload
(defun embark-by-type-act-set-current-thing ()
...
(defface embark-by-type-other-instance-face
  '((t :inherit lazy-highlight))
  "Face for highlighted other-instances of the captured target's type."
  :group 'embark-by-type)

(defvar-local embark-by-type--instance-overlays nil
  "Overlays painted by `embark-by-type-act-highlight-instances'.")

(defvar-local embark-by-type--highlights-enabled nil
  "Non-nil after `act-highlight-instances' fires; consumed by the
refresh-on-rotate advice.")

(defun embark-by-type--clear-instance-overlays ()
  "Remove all `embark-by-type-act-highlight-instances' overlays in
the current buffer."
  (mapc #'delete-overlay embark-by-type--instance-overlays)
  (setq embark-by-type--instance-overlays nil))

(defun embark-by-type--collect-instances-of-thing (thing)
...
`embark-by-type-act-highlight-instances'."
  (let ((results nil)
        (treesit-defined
...
;;;###autoload
(defun embark-by-type-act-highlight-instances ()
  "Highlight every other instance of the captured target's type.

Uses `embark-by-type-other-instance-face'.  Highlights persist
...
type changes."
  ...
  (cond
   ...
   (t
    (let* ((type embark-by-type-last-target-type)
           (thing (alist-get type embark-by-type-nav-type-map type))
           (own-bounds embark-by-type-last-target-bounds))
      (embark-by-type--clear-instance-overlays)
      (dolist (b (embark-by-type--collect-instances-of-thing thing))
        (unless (and own-bounds
                     (= (car b) (car own-bounds))
                     (= (cdr b) (cdr own-bounds)))
          (let ((ov (make-overlay (car b) (cdr b))))
            (overlay-put ov 'face 'embark-by-type-other-instance-face)
            (overlay-put ov 'embark-by-type-highlight t)
            (push ov embark-by-type--instance-overlays))))
      (setq embark-by-type--highlights-enabled t)))))

;;;###autoload
(defun embark-by-type-act-clear-highlights ()
  "Remove `embark-by-type-act-highlight-instances' overlays."
  (interactive)
  (embark-by-type--clear-instance-overlays)
  (setq embark-by-type--highlights-enabled nil))

(defun embark-by-type--refresh-highlights-on-rotate (rotated-targets)
  "Re-paint instance highlights when embark cycles to a new type.
Runs only when `embark-by-type--highlights-enabled' is on; otherwise
no-op.  Returns ROTATED-TARGETS unchanged."
  (when (and embark-by-type--highlights-enabled rotated-targets)
    (let* ((target (car rotated-targets))
...
;;;###autoload
(defun embark-by-type-act-select-as-region ()
...
;;;###autoload
(defun embark-by-type-act-narrow ()
...
(defvar-local embark-by-type--pick-preview-overlay nil
  "One-shot preview overlay for the consult-based pickers.")

(defun embark-by-type--pick-clear-preview ()
  "Delete `embark-by-type--pick-preview-overlay' if any."
  (when (overlayp embark-by-type--pick-preview-overlay)
    (delete-overlay embark-by-type--pick-preview-overlay))
  (setq embark-by-type--pick-preview-overlay nil))


;;;; Pick: target type (consult)
;; ----------------------------------------------------------------

(defun embark-by-type--pick-target-type-do (candidates)
  "Open `consult--read' to pick from CANDIDATES, re-enter
`embark-act' on the chosen target.  Preview paints the focused
candidate's bounds with `embark-by-type-other-instance-face'."
  (unwind-protect
      (let* ((choice
              (consult--read
               ...
               (lambda (action cand)
                 (pcase action
                   ('preview
                    (embark-by-type--pick-clear-preview)
                    (when (and cand (stringp cand))
                      (when-let* ((tgt (cdr (assoc cand candidates)))
                        ...
                        (let ((ov (make-overlay (car b) (cdr b))))
                          ...
                          (setq embark-by-type--pick-preview-overlay ov)
                          (goto-char (car b))))))))))
...
;;;###autoload
(defun embark-by-type-pick-target-type ()
...
consult UI opens cleanly after embark-act's prompt exits."
  ...
  (let* ((targets (embark--targets))
    ...
    (if (null candidates)
      ...
      (run-at-time 0 nil
                   #'embark-by-type--pick-target-type-do candidates))))


;;;; Pick: target type (single-key transient)
;; ----------------------------------------------------------------

(defun embark-by-type--assign-type-keys (types)
...
(defun embark-by-type--pick-target-type-key-do (choices key->type targets)
...
;;;###autoload
(defun embark-by-type-pick-target-type-key ()
  "Pick a target type at point via a single-key transient menu.

Like `embark-by-type-pick-target-type' but uses
...
embark-act's prompt exits."
  ...
  (let* ((targets (cl-remove-if-not
         ...
         (key->type (embark-by-type--assign-type-keys types))
         (choices (mapcar
                   (lambda (entry)
                     (let* ((c (car entry))
                            (type (cdr entry))
    ...
    (cond
     ...
     (t
      (run-at-time 0 nil
                   #'embark-by-type--pick-target-type-key-do
...
(defcustom embark-by-type-symbol-target-types
...
sexp-shaped things) and use `embark-by-type--collect-symbols-of-embark-type'
which walks symbols and classifies them by embark target type."
  :type '(repeat symbol)
  :group 'embark-by-type)

(defun embark-by-type--cheap-identifier-types (name)
...
(defun embark-by-type--collect-symbols-of-embark-type (target-type)
  "Walk every symbol in the buffer, return bounds of those whose
types include TARGET-TYPE per `embark-by-type--cheap-identifier-types'."
          ...
          (cond
           ((and bounds (> (cdr bounds) (or last-end 0)))
            (let* ((name (buffer-substring-no-properties
                   ...
                   (types (embark-by-type--cheap-identifier-types name)))
...
(defcustom embark-by-type-url-regex
  "\\b\\(?:https?\\|ftp\\|file\\)://[^ \t\n\r<>\"'`]+[^ \t\n\r<>\"'`.,;:!?)]"
  "Regex used by `embark-by-type-collect-bounds-by-regex' for URLs."
  :type 'regexp
  :group 'embark-by-type)

(defcustom embark-by-type-email-regex
  "\\b[[:alnum:]._%+-]+@[[:alnum:].-]+\\.[[:alpha:]]\\{2,\\}\\b"
  "Regex used by `embark-by-type-collect-bounds-by-regex' for emails."
  :type 'regexp
  :group 'embark-by-type)

(defun embark-by-type-collect-bounds-by-regex (regex beg end)
...
(defun embark-by-type-collect-bounds-by-scan (thing beg end)
...
(defun embark-by-type--compound-bound-p (b)
...
(defun embark-by-type-collect-org-links-of-scheme (scheme-regex beg end)
...
(defun embark-by-type-collect-visible-instances (type thing)
...
- Symbol-shaped types (per `embark-by-type-symbol-target-types') ->
...
- Else -> per-position thing-at-point walker."
  (let* ((win-beg (window-start))
    ...
    (cond
     ((eq type 'url)
      (embark-by-type-collect-bounds-by-regex
       embark-by-type-url-regex win-beg win-end))
     ((eq type 'org-url-link)
      (append
       (embark-by-type-collect-bounds-by-regex
        embark-by-type-url-regex win-beg win-end)
       (embark-by-type-collect-org-links-of-scheme
        "\\`\\(?:https?\\|ftp\\)://" win-beg win-end)))
     ((eq type 'email)
      (embark-by-type-collect-bounds-by-regex
       embark-by-type-email-regex win-beg win-end))
     ...
     ((eq type 'org-file-link)
      (append
       (embark-by-type-collect-bounds-by-scan 'filename win-beg win-end)
       (embark-by-type-collect-org-links-of-scheme
        "\\`\\(?:file:\\|attachment:\\|[./]\\)" win-beg win-end)))
     ((memq type embark-by-type-symbol-target-types)
      (cl-remove-if-not
       (lambda (b)
         (and (>= (cdr b) win-beg)
              (<= (car b) win-end)))
       (embark-by-type--collect-symbols-of-embark-type type)))
     (t
      (let ((bounds (embark-by-type-collect-bounds-by-scan
                     thing win-beg win-end)))
        (if (memq thing '(sexp embark-expression))
            (cl-remove-if-not #'embark-by-type--compound-bound-p bounds)
          bounds))))))
...
(defun embark-by-type--pick-instance-do (type _thing candidates start)
...
;;;###autoload
(defun embark-by-type-pick-instance ()
  "Pick an instance of the captured target's type via consult.
Uses `embark-by-type-collect-visible-instances' to gather targets."
  (interactive)
  (embark-by-type--require-capture-mode)
  (let* ((type embark-by-type-last-target-type)
         (thing (and type (alist-get type embark-by-type-nav-type-map type)))
         (start (point)))
    (cond
     ((null type)
      (message "No embark target captured yet"))
     (t
      (let* ((instances (embark-by-type-collect-visible-instances type thing))
             (candidates
              (mapcar (lambda (b)
        ...
        (if (null candidates)
          ...
          (run-at-time 0 nil
                       #'embark-by-type--pick-instance-do
...
(defun embark-by-type--avy-pick-bounds (instances)
...
(defun embark-by-type--avy-act (_type bounds)
  "Jump point to BOUNDS' start.  Avy entry points are nav-only."
  (goto-char (car bounds)))

(defun embark-by-type--avy-pick-instance-do (type thing start)
  ...
  (let ((instances (embark-by-type-collect-visible-instances type thing)))
    (cond
     ((null instances)
      ...
      (goto-char start))
     (t
      (if-let* ((bounds (embark-by-type--avy-pick-bounds instances)))
          (embark-by-type--avy-act type bounds)
        (goto-char start))))))

;;;###autoload
(defun embark-by-type-avy-pick-instance ()
  ...
  (let* ((type embark-by-type-last-target-type)
    ...
    (cond
     ...
     (t
      (run-at-time 0 nil
                   #'embark-by-type--avy-pick-instance-do
                   type thing start)))))
...
(defun embark-by-type--jump-to-type-closest (instances anchor)
...
(defun embark-by-type--jump-to-type-paint (instances)
  "Paint INSTANCES with `embark-by-type-other-instance-face' overlays."
  (let (overlays)
    (dolist (b instances)
      (let ((ov (make-overlay (car b) (cdr b))))
        (overlay-put ov 'face 'embark-by-type-other-instance-face)
...
(defun embark-by-type--jump-type-applicable-p (sym)
...
;;;###autoload
(defun embark-by-type-jump-to-type ()
...
want without already being at an embark target."
  ...
  (let* ((all-things (delete-dups
                      (append
                       (mapcar #'cdr embark-by-type-nav-type-map)
                       embark-by-type-symbol-target-types)))
         (applicable (cl-remove-if-not
                      #'embark-by-type--jump-type-applicable-p
                      all-things))
         (choice (and applicable
                      (intern (completing-read
                               ...
                               nil t)))))
    (when choice
      (let* ((instances (embark-by-type-collect-visible-instances
                         choice choice))
             (candidates
              (mapcar (lambda (b)
                        (cons (truncate-string-to-width
        ...
        (if (null candidates)
          ...
          (embark-by-type--pick-instance-do choice choice
                                            candidates start))))))

;;;###autoload
(defun embark-by-type-avy-jump-to-type ()
  ...
  (let* ((all-things (delete-dups
         ...
         (start (point)))
    (when choice
      (embark-by-type--avy-pick-instance-do choice choice start))))


;;;; Default bindings + setup
;; ----------------------------------------------------------------

(defcustom embark-by-type-default-bindings
  '(("C-j" . embark-by-type-nav-next)
    ("C-k" . embark-by-type-nav-prev)
    ("C-a" . embark-by-type-nav-beg)
    ("C-e" . embark-by-type-nav-end)
    ("C-t" . embark-by-type-act-set-current-thing)
    ("C-f" . embark-by-type-act-focus)
    ("C-v" . embark-by-type-act-select-as-region)
    ("C-n" . embark-by-type-act-narrow)
    ("C-l" . embark-by-type-pick-target-type)
    (";"   . embark-by-type-pick-target-type-key)
    ("C-o" . embark-by-type-pick-instance)
    ("C-;" . embark-by-type-avy-pick-instance)
    ("*"   . embark-by-type-act-highlight-instances))
  "Recommended bindings installed by `embark-by-type-install-default-bindings'."
  :type '(alist :key-type string :value-type symbol)
  :group 'embark-by-type)

;;;###autoload
(defun embark-by-type-install-default-bindings ()
  "Install `embark-by-type-default-bindings' into `embark-general-map'."
  (interactive)
  (dolist (b embark-by-type-default-bindings)
    (define-key embark-general-map (kbd (car b)) (cdr b))))

;;;###autoload
(defun embark-by-type-setup ()
  "One-call setup: enable capture-mode + sort-by-bounds-mode +
back-cycle-mode, install default bindings, and register
`embark-by-type-defun-map' for `defun' targets."
  (interactive)
  (embark-by-type-capture-mode 1)
  (embark-by-type-sort-by-bounds-mode 1)
  (embark-by-type-back-cycle-mode 1)
  (embark-by-type-install-default-bindings)
  (setf (alist-get 'defun embark-keymap-alist) 'embark-by-type-defun-map)
  ;; Refresh-highlights advice piggybacks on capture-mode.
  (advice-add 'embark--rotate :filter-return
              #'embark-by-type--refresh-highlights-on-rotate))


(provide 'embark-by-type)
