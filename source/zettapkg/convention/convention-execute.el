(defun convention-get-repl-buffers ()
  ...
  (seq-filter
   (function (lambda (x) (and
                     (not (string-match-p (regexp-quote "tramp") x))
...
(defun convention-named-async ()
  ...
  (let ((cmd (completing-read "Please enter a command: " '(
                                            ...
                                            "source ~/envs/dagit_tutorial/bin/activate && export DAGSTER_HOME=/Users/cbaker63 && dagster-celery worker start -A dagster_celery.app"
                                            )))
...
(defun convention-associate-text-buffer-to-program (arg)
  ...
  (let ((program-name (if (or arg (not (boundp 'convention-program-target)))
                          (completing-read "Please select a program: " '(
                                                                             ...
                                                                             "source ~/envs/dagit_tutorial/bin/activate && export DAGSTER_HOME=/Users/cbaker63 && dagit -f"
                        ...
                        convention-program-target)))
    (setq-local convention-program-target program-name)
...
(defun convention-exec-as-file (arg &optional region)
  ...
  (let*  (
    ...
    (message "program ")
    (message convention-program-target)
