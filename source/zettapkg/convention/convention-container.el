...
(defun convention-dock ()
  (interactive)
  "A function to conveniently ssh into a docker container"
  (let* ((host (completing-read "Choose a container: " (convention-list-running-containers)))
         (default-directory (concat
                             "/"
                             (4mn-get-tramp-remote-part)
                             (when (not (string= "" (4mn-get-tramp-remote-part))) "|")
                             "docker:"
                             host
                             ":/"))
         )
...
(defun convention-dock-from-docker-container-mode ()
  (interactive)
  "A function to conveniently ssh into a docker container"
  (let* ((host (elt (tabulated-list-get-entry) 6))
         (default-directory (concat
                             "/"
                             (4mn-get-tramp-remote-part)
                             (when (not (string= "" (4mn-get-tramp-remote-part))) "|")
